...
const FAQS = [
  ...
  { q: 'Can I cancel anytime?', a: 'Yes. No lock-in contracts. Cancel anytime from your dashboard.' },
];
  ...
  <section id="faq" className="py-32 relative z-10">
    <div className="container mx-auto px-6 max-w-2xl">
      ...
      <ScrollReveal delay={100}>
        <div className="glass-card !p-2 !hover:transform-none">
          <Accordion type="single" collapsible className="w-full">
            {FAQS.map((faq, i) => (
              <AccordionItem key={i} value={`faq-${i}`} className="border-border/30 px-4">
                <AccordionTrigger className="text-sm text-foreground hover:text-primary transition-colors py-5 font-medium">
                  {faq.q}
                </AccordionTrigger>
