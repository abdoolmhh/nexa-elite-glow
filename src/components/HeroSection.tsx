...
const HeroSection = () => {
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      section.style.setProperty('--cx', `${e.clientX - rect.left}px`);
      section.style.setProperty('--cy', `${e.clientY - rect.top}px`);
    };
  ...
  return (
    <section
      ...
      style={{ background: 'radial-gradient(circle at var(--cx, 50%) var(--cy, 30%), hsl(158 82% 50% / 0.06) 0%, transparent 50%)' }}
    >
      {/* Morphing blobs */}
      ...
      <div className="container mx-auto px-6 grid lg:grid-cols-2 gap-16 xl:gap-24 items-center relative z-10">
        ...
        <div className="max-w-xl">
          ...
          <h1 className="hero-reveal-2 text-foreground mb-6" style={{ fontSize: 'clamp(40px, 6vw, 68px)', fontWeight: 800, letterSpacing: '-0.05em', lineHeight: 1.02 }}>
            Run Your Store<br />
            <span className="text-primary">Smarter.</span>
          </h1>

          <p className="hero-reveal-3 text-lg text-muted-foreground mb-10 leading-relaxed max-w-[440px]">
            AI-powered POS & inventory for Nigerian retailers. Sell faster, eliminate stockouts, grow profits automatically.
          </p>

          <div className="hero-reveal-4 flex flex-col sm:flex-row gap-3 mb-14">
            <Link to="/try" className="btn-primary cta-glow text-center group">
        ...
        <div className="relative hidden lg:flex hero-reveal-6 justify-center perspective-container">
          <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
            <div className="w-3/4 h-3/4 rounded-full blur-[120px] morph-blob" style={{ background: 'hsl(158 82% 50% / 0.14)' }} />
          </div>

          <div className="relative float-gentle w-full max-w-[580px]" style={{ transform: 'perspective(1000px) rotateY(-4deg) rotateX(3deg)' }}>
            <div className="rounded-2xl overflow-hidden shadow-2xl shadow-foreground/10 border border-border bg-card/40 backdrop-blur-sm glow-pulse gradient-border">
              <img src={heroDashboard} alt="NEXA StoreOS Dashboard — real-time analytics" className="w-full" />
              <div className="laser-scan" />
            </div>

            {/* Revenue card */}
            <div className="absolute -top-5 -right-6 glass-card !p-4 float-slow !hover:transform-none">
              <p className="text-xs text-muted-foreground font-medium">Today's Revenue</p>
              <p className="text-2xl font-bold text-foreground" style={{ letterSpacing: '-0.03em', fontFamily: "'Space Grotesk'" }}>₦248,500</p>
              <p className="text-xs text-primary font-bold">↑ 18.4% vs yesterday</p>
            </div>

            {/* AI card */}
            <div className="absolute -bottom-5 -left-6 glass-card !p-4 float-delayed !hover:transform-none">
          ...
          {COUNTERS.map((c) => {
            ...
            return (
              <div key={c.label} ref={ref} className="text-center">
                <div className="text-2xl md:text-3xl font-bold text-foreground" style={{ letterSpacing: '-0.04em', fontFamily: "'Space Grotesk'" }}>
